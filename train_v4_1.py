...
# -*- coding: utf-8 -*-
"""
RF-DETR Segmentation Training Script (cell_opti_10) - v4.1

핵심 기능 (v4.1 포함):
- num_queries 변경 지원 (--num-queries)
- num_select 변경 지원 (--num-select)  # inference 상한(PostProcess top-k) 제어
...
def build_model(
    ...
    for cls_name in candidates:
        ...
        try:
            # NOTE:
            # RFDETRSeg* 클래스들은 __init__ 시그니처가 (**kwargs)로 노출되는 경우가 많아서
            # inspect.signature로 파라미터 유무를 판별하면 num_queries/num_select를 못 넣게 된다.
            # → 여기서는 그냥 kwargs로 넣고, wrapper가 받아서 config에 반영하도록 한다.
            ...
            # ✅ inference 결과 상한(PostProcess top-k) 관련
            if num_queries is not None:
                kwargs["num_queries"] = int(num_queries)
            if num_select is not None:
                kwargs["num_select"] = int(num_select)

            # pretrained weight 로딩 제어
            # - None  : 기본값(각 config의 default pretrain_weights)을 그대로 사용
            # - ""/"none"/"None" : 명시적으로 로딩 끔
            # - path  : 해당 경로로 pretrained 로딩
            if pretrain_weights is not None:
                if str(pretrain_weights).lower() in {"", "none", "null"}:
...
class TrainConfig:
    ...
    # v4.1 추가: wrapper의 pretrained 자동 로딩을 제어
    pretrain_weights: Optional[str]


def parse_args() -> TrainConfig:
    p = argparse.ArgumentParser(description="RF-DETR Segmentation training (cell_opti_10) - v4.1")
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="PostProcess top-k (inference 결과 상한). 보통 num_queries와 동일하게 둠",
    )
    ...
    # v4.1: wrapper 내부 pretrained 자동 로딩 제어
    # - 미지정(None): 각 모델 config의 기본 pretrain_weights를 사용
    # - 'none': pretrained 로딩 끔
    # - 경로: 해당 경로로 pretrained 로딩
    p.add_argument(
        "--pretrain-weights",
        type=str,
        default=None,
        help="wrapper pretrain_weights override (e.g. /path/to/rf-detr-seg-xxlarge.pt or 'none')",
    )
...
def main():
    ...
    if cfg.num_select is not None:
        model_tag = f"{model_tag}__S{cfg.num_select}"
    ...
    print("====================================================")
    print("[INFO] RF-DETR Seg Training Start (v4.1)")
    ...
    print(f"[INFO] num_select        : {cfg.num_select}")
    ...
    print(f"[INFO] pretrain_weights  : {cfg.pretrain_weights}")
    ...
    # 모델 생성
    # - num_queries: DETR query 개수(encoder/decoder proposals 상한)
    # - num_select : PostProcess top-k (✅ 최종 출력 인스턴스 상한)
    # - partial_load를 쓸 때는 wrapper의 자동 pretrained 로딩을 꺼서(=none)
    #   아래에서 shape-compatible 텐서만 주입하도록 한다.
    pretrain_w = cfg.pretrain_weights
    if cfg.partial_load:
        pretrain_w = "none"
