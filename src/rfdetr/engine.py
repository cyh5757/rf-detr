...
def evaluate(model, criterion, postprocess, data_loader, base_ds, device, args=None):
    should_profile_vram = bool(getattr(args, "profile_vram", False))
    if should_profile_vram and torch.cuda.is_available():
    ...
    if should_profile_vram and torch.cuda.is_available():
        ...
        cur_reserved = torch.cuda.memory_reserved() / 1024**2
        peak_alloc = torch.cuda.max_memory_allocated() / 1024**2
        peak_reserved = torch.cuda.max_memory_reserved() / 1024**2
        logger.info(
            "[VRAM][EVAL] current_alloc=%.2fMB current_reserved=%.2fMB peak_alloc=%.2fMB peak_reserved=%.2fMB",
            cur_alloc,
            cur_reserved,
            peak_alloc,
            peak_reserved,
