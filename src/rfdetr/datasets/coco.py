...
def make_coco_transforms(image_set: str, resolution: int, multi_scale: bool = False, expanded_scales: bool = False, skip_random_resize: bool = False, patch_size: int = 16, num_windows: int = 4, pad_to_square: bool = False) -> T.Compose:
    ...
    if image_set == 'val_speed':
        square_resize = T.SquarePadResize if pad_to_square else T.SquareResize
        return T.Compose([
            square_resize([resolution]),
            normalize,
        ])
...
def make_coco_transforms_square_div_64(image_set: str, resolution: int, multi_scale: bool = False, expanded_scales: bool = False, skip_random_resize: bool = False, patch_size: int = 16, num_windows: int = 4, pad_to_square: bool = False) -> T.Compose:
...
def build_coco(image_set: str, args: Any, resolution: int) -> CocoDetection:
    ...
    pad_to_square = getattr(args, "pad_to_square", False)
    include_masks = getattr(args, "segmentation_head", False)

    if square_resize_div_64:
        if pad_to_square:
            logger.info(f"Building COCO {image_set} dataset with square pad-resize at resolution {resolution}")
        else:
...
def build_roboflow_from_coco(image_set: str, args: Any, resolution: int) -> CocoDetection:
    ...
    if square_resize_div_64:
        if pad_to_square:
            logger.info(f"Building Roboflow {image_set} dataset with square pad-resize at resolution {resolution}")
        else:
