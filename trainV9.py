...
# -*- coding: utf-8 -*-
"""
RF-DETR Segmentation Training Script (trainV9)

핵심:
- trainV8 기반 학습 파이프라인 유지
- 학습 전 COCO 데이터셋을 타일로 분할(이미지 + segmentation annotation clip)
- 인스턴스 과밀 이미지를 완화해 query/select cap 영향 감소

실행 예:
  cd /home/mbd1234/rf-detr
  python trainV9.py --size M --resolution 432 --tile-enable --tile-overlap 0.20 --sanity-check
"""
...
def default_tile_output_root(data_root: Path, tile_size: int, tile_overlap: float) -> Path:
    ov = int(round(tile_overlap * 100))
    return data_root.parent / f"{data_root.name}_tiled_r{tile_size}_ov{ov:02d}"
...
def parse_args() -> TrainConfig:
    p = argparse.ArgumentParser(description="RF-DETR Segmentation training (trainV9, tiled dataset)")
    ...
    tile_output_root = (
        ...
        else default_tile_output_root(data_root, tile_size=tile_size, tile_overlap=a.tile_overlap)
    )
...
def main():
    ...
    print("====================================================")
    print("[INFO] RF-DETR Seg Training Start (trainV9)")
